...
下一步优先检查 Elastic Net 内层选参路径，并预先固定扩展 α 上边界的训练内敏感性实验，使用逐靶点数值证书控制求解精度；完成后再决定是否重算外部比较。独立环境重跑仍未完成。若继续研究 DeepDEP，应另行固定更完整表达特征和匹配优化预算的实验，不能根据现有留出表现反复调参后再称独立验证。当前没有证据支持用复杂模型替换 PCR-ridge，新模型不得再次使用已经访问的 Locked Test 进行选择或确认。
