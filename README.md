...
目的：将最终冻结分析整理为可直接审阅和导出的科学图件，同时保留不确定性、证据冲突和患者功能真值缺失，不通过视觉设计夸大候选强度。

范围：图 1 汇总内部完整癌系留出、Sanger 外部复现、患者迁移稳定性和证据递减；图 2 按原 discovery rank 展示 20 个候选的支持证据、直接反向证据和正常肾类器官风险，不生成综合分数；图 3 将 DRIVE ccRCC 依赖强度及其 95% bootstrap 区间与 GTEx 正常肾暴露并列。锁定 TCGA Test 未使用，图件不改变候选分类或排序。
