...
下一步优先完成复现可信度检查：固定当前结果，在独立环境核验输入与结果快照，并针对真实训练折检查 Elastic Net 数值最优性。若继续研究 DeepDEP，应预先固定使用更完整表达特征和匹配优化预算的敏感性实验，把它作为新的开发实验单独报告；不要根据现有留出表现反复调参后再称独立验证。当前没有证据支持用复杂模型替换 PCR-ridge，新模型不得再次使用已经访问的 Locked Test 进行选择或确认。
